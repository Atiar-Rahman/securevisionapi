...
model_path = os.path.join(
    BASE_DIR,
    "ml",
    "best_cnn_lstm_model_rgb_version1.onnx"
)
...
def warmup_model():
    dummy_input = np.zeros(
        (1, SEQ_LEN, IMG_SIZE, IMG_SIZE, 3),
        dtype=np.float32
    )

    session.run(None, {input_name: dummy_input})

    logger.info("RGB ONNX model warmup complete")
...
def _cleanup_camera_state(now=None, *, force=False):
    ...
    for camera_id in stale_camera_ids:
        ...
        last_prediction_debug.pop(f"camera:{camera_id}:rgb", None)
        last_prediction_debug.pop("video:rgb", None)
...
def get_sequence_status_rgb(camera_id="default"):
    buffered_frames = len(camera_buffers.get(camera_id, ()))
    return {
        "buffered_frames": buffered_frames,
        "required_frames": SEQ_LEN,
        "sequence_ready": buffered_frames >= SEQ_LEN,
    }


def predict_frame_multi_rgb(frame, camera_id="default"):
    ...
    with lock:
        ...
        try:
            ...
            logger.exception(
                "RGB preprocessing error for camera %s",
                camera_id
            )
            return None, None
        ...
        try:
            ...
            logger.exception(
                "RGB ONNX prediction error for camera %s",
                camera_id
            )
            return None, None
...
def predict_frame_rgb(frame, camera_id="default", skip_rate=None):
    return predict_frame_multi_rgb(frame, camera_id)


def run_video_prediction_rgb(
    video_path,
    ...
    while True:
        ...
        try:
            ...
            logger.exception("RGB frame preprocessing error")
            continue
        ...
        try:
            ...
            last_prediction_debug["video:rgb"] = {
                "suspicious_score": float(suspicious_score),
                "threshold": float(SUSPICIOUS_THRESHOLD),
                "label": label,
                "confidence": float(confidence),
            ...
            logger.exception("RGB ONNX prediction error")
            continue
        ...
        if label == "Suspicious":
            ...
            if DEBUG:
                print(f"[VIDEO RGB] Suspicious vote {suspicious}")
        else:
            normal += 1

            if DEBUG:
                print(f"[VIDEO RGB] Normal vote {normal}")

        if stop_on_suspicious and suspicious >= MIN_SUSPICIOUS_VOTES:
            break

    cap.release()

    final = "Suspicious" if suspicious >= MIN_SUSPICIOUS_VOTES else "Normal"

    print("\n====================================")
    print("FINAL RGB VIDEO RESULT")
    print("------------------------------------")
